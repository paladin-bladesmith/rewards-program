...
async fn fail_token_account_incorrect_owner() {
    ...
    // Set up a token account with incorrect owner.
    {
        context.set_account(
            &token_account,
            &AccountSharedData::new_data(100_000_000, &vec![5; 165], &Pubkey::new_unique())
...
async fn success() {
    ...
    .await;
    setup_mint(&mut context, &mint, &Pubkey::new_unique(), mint_supply).await;
    ...
    assert_eq!(
        ...
        &HolderRewards {
            ...
            unharvested_rewards: (token_account_balance / mint_supply) * total_rewards,
        },
    );
}
