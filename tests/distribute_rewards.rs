...
async fn fail_payer_not_signer() {
    ...
    let mut instruction = distribute_rewards(&holder_rewards_pool, &payer.pubkey(), amount);
...
async fn fail_holder_rewards_pool_incorrect_owner() {
    ...
    let instruction = distribute_rewards(&holder_rewards_pool, &payer.pubkey(), amount);
